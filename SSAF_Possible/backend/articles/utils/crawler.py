...
def crawl_data():
    ...
    chrome_options.add_argument("--headless")  # 브라우저를 화면에 표시하지 않습니다.
    ...
    for i in range(1, 6):
        ...
        for k in item:
            ...
            article_id=k.find('a')['href'].split('/')[-2]+'woowahan'
            ent_name = 'woowahan'
            published_date = k.find('a').find('p')[0].get_text()
            link = k.find('a')['href']
            author = published_date = k.find('a').find('p')[1].get_text()
            content = k.find('a').find('p').get_text()
            print(title, article_id, published_date, link, ent_name, author, content)
        next = driver.find_element_by_xpath('/html/body/div[3]/div[2]/div[2]/div[1]/div[21]/div/a[5]')
        next.click()
    return
